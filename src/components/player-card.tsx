...
import { age, type PublicPlayer } from "@/lib/public-data";
  ...
  return (
    <Link href={`/players/${player.slug}`} className="flex items-center gap-3 rounded-md border border-zinc-200 bg-white p-3 shadow-sm">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="truncate text-sm text-zinc-600">#{player.jerseyNumber} {player.position} · {player.teamName ?? "Unassigned"} · {age(player.dateOfBirth)}</p>
      </div>
      <div className="text-right">
        <p className="font-mono text-lg font-bold text-emerald-700">{player.stats.goals}</p>
